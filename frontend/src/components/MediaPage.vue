<template>
  ...
  <section class="main-detail-intro pb-0" v-if="movie">
    <div class="container">
      <div class="row rtl">
        ...
        <div
          class="col-md-12 col-10 text-center"
          v-if="movie.imdb_id"
        >
          <a href="#playContent" class="play-video" v-on:click="play()">
            <div class="overlay">
              <img
                class="rounded shadow-lg"
                v-bind:src="'http://localhost:1337' + movie.poster.url"
                alt=""
              />
        ...
        <div class="details col-md-8 text-center d-flex flex-column">
          <h2 class="pt-5 pb-2">{{ movie.title }}</h2>
          <ul class="list-inline px-0">
            ...
            <li class="font-weight-bold pt-3">
              <span class="px-2 rounded bg-warning text-dark"
                >10 / {{ movie.rating.rate }}</span
              >
              از {{ movie.rating.rates_amount }} رای
              <img
                class="mr-3"
  ...
  <section id="playContent" class="pt-5" v-if="playMedia">
    <div class="container">
      <Play v-bind:source="movie.sources"> </Play>
    </div>
  </section>
      ...
      <div class="row rtl d-flex my-5">
        ...
        <div class="col-md-4 p-3">
          <ul class="list-group list-group-flush m-0 p-0 pb-3" v-if="directors">
            ...
            >
            <router-link :to="'/cast/director/' + director.imdb_id" class="text-dark">
          ...
          <ul class="list-group list-group-flush m-0 p-0" v-if="stars">
            ...
            <router-link :to="'/cast/star/' + star.imdb_id" class="text-dark">
              <span class="avatar pr-3" v-if="star.head_shot">
                <img :src="'http://localhost:1337' + star.head_shot.url" />
              </span>
              {{ star.name }}
            </router-link>
            </li>
...
export default {
  ...
  methods: {
    play: function() {
      if(this.logged_in) {
        if(this.movie.free) {
          ...
          if(this.movie.sources == null) {
            this.$router.push({ name: 'Subscribtion' })
    ...
    setProperties: function (data) {
      this.imdb_id = data.movies[0].imdb_id;
      this.movie = data.movies[0];
      this.stars = this.movie.crew.stars;
      this.directors = this.movie.crew.directors;
      this.movie.genres = this.movie.genres.map((genre) => genre.title);
      this.movie.countries = this.movie.countries.map(
      ...
      if (data.me) {
        ...
        this.archivedList = data.me.user.archived_movies;
      ...
      fetch("http://localhost:1337/graphql", {
        ...
        body: JSON.stringify({
          query:
            ...
            '"}){id ,imdb_id ,title , free ,runtime ,poster {url} ,summary ,year ,budget ,genres {title} ,countries {name} ,language ,crew {stars{imdb_id , name , head_shot{url}} , directors{imdb_id , name , head_shot {url}}} ,rating{rate , rates_amount},sources{videos{url , quality}}} , me{id , user{archived_movies , loved}}}',
        }),
        headers: {
